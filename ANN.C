...
//  traits_struct bad;
  ...
  //tree->Branch("EventInfo",&this_event_info,"RunNumber/I:EventNumber");
...
//have to initialize the variables in the struct
   ...
   Long64_t nentries =15; //fChain->GetEntriesFast();//****test change
      ...
      for (int i= 0; i<10;i++){
       ...
       /*for (int i= 0; i<15; i++){      
         ...
         for(int k=1; k<100; k++){
            ...
            if (-0.1< vhwidth1.at(i)->GetBinCenter(k) < 0.1)
            {*/
